...
class Handler :
    ...
    override fun handleRequest(event: APIGatewayV2HTTPEvent, context: Context): APIGatewayV2HTTPResponse {
        ...
        val x = event.pathParameters["x"]!!.toInt()
        val y = event.pathParameters["y"]!!.toInt()
        val z = event.pathParameters["z"]!!.toInt()
        val seed = event.pathParameters["seed"]!!.toDouble()
        val mapTileKey = MapTileKey(z, x, y, seed)
        val planet = FractalPlanet(seed)
        val tile = planet.getMapTile(mapTileKey)
        val pngByteArray = tile.pngByteArray
